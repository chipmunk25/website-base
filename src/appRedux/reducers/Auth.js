import {
  ...
  SUCCESS_GET_USERS,
  SUCCESS_SAVE_BRANCH,
  SUCCESS_UPDATE_BRANCH,
  SUCCESS_DELETE_BRANCH,
  SUCCESS_SAVE_USER,
  SUCCESS_DELETE_USER,
  SUCCESS_UPDATEUSERS,
  SUCCESS_SMSBAL,
  ...
  SUCCESS_UPDATE_ROLE,
  SUCCESS_DELETE_ROLE,
  SUCCESS_GET_PERMISSION,
  SUCCESS_SAVE_PERMISSION,
...
const Auth = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case SUCCESS_UPDATEUSERS: {
      index = state.userLists.indexOf(
        state.userLists.find(
          (item) => parseInt(action.payload.id) === parseInt(item.id)
        )
      );
    ...
    case SUCCESS_UPDATE_BRANCH:
      index = state.branchLists.indexOf(
        state.branchLists.find(
          (item) => parseInt(action.payload.id) === parseInt(item.id)
        )
      );
    ...
    case SUCCESS_SAVE_BRANCH:
      return {
        ...state,
        branchLists: [...state.branchLists, { ...action.payload }],
      };
    case SUCCESS_DELETE_BRANCH:
      return {
        ...state,
        branchLists: state.branchLists.filter(
    ...
    case SUCCESS_SAVE_ROLE:
      return {
        ...state,
        roleLists: [...state.roleLists, { ...action.payload }],
      };
    case SUCCESS_UPDATE_ROLE:
      index = state.roleLists.indexOf(
        state.roleLists.find(
          (item) => parseInt(action.payload.id) === parseInt(item.id)
        )
      );
    ...
    case SUCCESS_SAVE_PERMISSION:
      return {
        ...state,
        permissionLists: [{ ...action.payload }, ...state.permissionLists],
      };
    case SUCCESS_UPDATE_PERMISSION:
      index = state.permissionLists.indexOf(
        state.permissionLists.find(
          (item) => parseInt(action.payload.id) === parseInt(item.id)
        )
      );
    ...
    case SUCCESS_DELETE_PERMISSION:
      return {
        ...state,
        roleLists: state.permissionLists.filter(
          (item) => parseInt(item.id) !== parseInt(action.payload.id)
        ),
      };
    default:
